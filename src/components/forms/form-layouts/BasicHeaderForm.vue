...
const getVendorItems = async () => {
  try {
    vendorItems.value = await fetchVendorItems();
  } catch (error) {
...
watch(selectedVendorItem, (newItem) => {
  if (newItem) {
    // Perbarui item terakhir dengan data dari barang yang dipilih
    const lastItemIndex = items.value.length - 1;

    if (lastItemIndex >= 0) {
      items.value[lastItemIndex].barang = newItem.ItemName;
      items.value[lastItemIndex].harga = newItem.Price;
      items.value[lastItemIndex].type = newItem.Unit;
    } else {
      // Jika tidak ada item di array, tambahkan item baru
      items.value.push({
